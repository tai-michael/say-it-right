...
export default function (transcriptWords: string[], testedWord: string) {
  ...
  return (
    ...
    checkForWordStemMatch(transcriptWords, testedWord)
  )
}
...
const checkForWordStemMatch = (transcriptWords: string[], testedWord: string) => {
  ...
  for (const sentenceWord of sentenceWords) {
    ...
    if (matchesTranscriptWord && sentenceWordStem === testedWordStem) return sentenceWord
  }
  return ''
}
